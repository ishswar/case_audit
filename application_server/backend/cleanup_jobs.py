#!/usr/bin/env python3
"""
Script to clean up duplicate reused entries in the all_jobs.json file.
This script can be run directly to perform a one-time cleanup.
"""
...
def main():
    ...
    # First pass: find the first reused entry for each case number
    for job_id, job_info in jobs.items():
        if job_id.startswith("reused_") and "case_number" in job_info:
            case_number = job_info["case_number"]
            if case_number not in case_to_job_map:
                case_to_job_map[case_number] = job_id
            else:
                # This is a duplicate reused entry
                jobs_to_remove.append(job_id)
    
    # Second pass: remove duplicates
    for job_id in jobs_to_remove:
        if job_id in jobs:
            print(f"Removing duplicate reused entry: {job_id}")
            del jobs[job_id]
    
    # Save the cleaned jobs file
    if jobs_to_remove:
        print(f"Removed {len(jobs_to_remove)} duplicate reused entries")
        try:
            # Make a backup first
        ...
        print("No duplicate reused entries found")
